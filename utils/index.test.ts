...
describe('getTenantSearchFeeMonthly', () => {
  it('should compute monthly amortized agency fee for 1 month over 3 years', () => {
    // (1 × 900) / (3 × 12) = 25
    expect(getTenantSearchFeeMonthly('900', '1', '3')).toBeCloseTo(25, 5);
  });

  it('should handle 1.2 months over 3 years', () => {
    // (1.2 × 900) / 36 = 30
    expect(getTenantSearchFeeMonthly('900', '1.2', '3')).toBeCloseTo(30, 5);
  });

  it('should handle 1.5 months over 3 years', () => {
    // (1.5 × 900) / 36 = 37.5
    expect(getTenantSearchFeeMonthly('900', '1.5', '3')).toBeCloseTo(37.5, 5);
  });

  it('should scale with rent (linear)', () => {
    const a = getTenantSearchFeeMonthly('1000', '1', '3');
    const b = getTenantSearchFeeMonthly('2000', '1', '3');
    expect(b).toBeCloseTo(a * 2, 5);
  });

  it('should scale inversely with tenancy duration', () => {
    const short = getTenantSearchFeeMonthly('900', '1', '1');
    const long = getTenantSearchFeeMonthly('900', '1', '6');
    expect(short).toBeCloseTo(long * 6, 5);
  });

  it('should return 0 for zero months', () => {
    expect(getTenantSearchFeeMonthly('900', '0', '3')).toBe(0);
  });

  it('should return 0 for zero duration (avoid div by zero)', () => {
    expect(getTenantSearchFeeMonthly('900', '1', '0')).toBe(0);
  });

  it('should return 0 for negative duration', () => {
    expect(getTenantSearchFeeMonthly('900', '1', '-1')).toBe(0);
  });

  it('should return 0 for zero rent', () => {
    expect(getTenantSearchFeeMonthly('0', '1', '3')).toBe(0);
  });

  it('should handle NaN inputs', () => {
    expect(getTenantSearchFeeMonthly('invalid', '1', '3')).toBe(0);
    expect(getTenantSearchFeeMonthly('900', 'invalid', '3')).toBe(0);
    expect(getTenantSearchFeeMonthly('900', '1', 'invalid')).toBe(0);
  });
});

describe('getNetMonthlyIncomeDetailed with tenant search fee', () => {
  it('should match base behavior when tenant search fee is 0 (default)', () => {
    // rent=900, charges=120, tax=1000 (83.33/mo), insurance=0, mgmt=8%, maint=0, vac=5%
    // effRent = 900*0.95 = 855, mgmt = 855*0.08 = 68.4
    // net = 855 - 120 - 83.33 - 0 - 68.4 - 0 = 583.27 ≈ 583
    const baseline = getNetMonthlyIncomeDetailed('900', '120', '1000', '0', '8', '0', '5');
    // Explicit 0 months should match the default
    const withZeroFee = getNetMonthlyIncomeDetailed('900', '120', '1000', '0', '8', '0', '5', 0, 0, 3);
    expect(withZeroFee).toBe(baseline);
  });

  it('should deduct the amortized agency fee when set', () => {
    // tsf = (1 * 900) / 36 = 25
    // baseline net ≈ 583, with -25 fee ≈ 558
    const baseline = Number(getNetMonthlyIncomeDetailed('900', '120', '1000', '0', '8', '0', '5'));
    const withFee = Number(getNetMonthlyIncomeDetailed('900', '120', '1000', '0', '8', '0', '5', 0, 1, 3));
    expect(baseline - withFee).toBeCloseTo(25, 0);
  });

  it('should deduct more with higher months coefficient', () => {
    // Use 4-decimal precision to avoid rounding artifacts in the diff
    const oneMonth = Number(getNetMonthlyIncomeDetailed('900', '120', '1000', '0', '8', '0', '5', 4, 1, 3));
    const oneHalfMonth = Number(getNetMonthlyIncomeDetailed('900', '120', '1000', '0', '8', '0', '5', 4, 1.5, 3));
    // tsf(1) = 25, tsf(1.5) = 37.5 → diff = 12.5
    expect(oneMonth - oneHalfMonth).toBeCloseTo(12.5, 4);
  });
});

describe('getBreakEvenRent with tenant search fee', () => {
  it('should match base behavior when tenant search fee is 0 (default)', () => {
    const baseline = getBreakEvenRent('150', '1200', '870', '5', '8', '0');
    const withZero = getBreakEvenRent('150', '1200', '870', '5', '8', '0', 0, 0, 3);
    expect(withZero).toBe(baseline);
  });

  it('should require a higher rent to break even when agency fee is added', () => {
    // costs=150, tax=1200(100/mo), mortgage=870, vacancy=5%, mgmt=8%, capex=0, tsf=1 month/3 yr
    // denominator without tsf = 0.95 * 0.92 - 0 = 0.874
    // denominator with tsf = 0.874 - 1/36 ≈ 0.846
    // required rent goes up by ~3.3%
    const baseline = Number(getBreakEvenRent('150', '1200', '870', '5', '8', '0'));
    const withFee = Number(getBreakEvenRent('150', '1200', '870', '5', '8', '0', 0, 1, 3));
    expect(withFee).toBeGreaterThan(baseline);
  });

  it('should return 0 if denominator becomes non-positive', () => {
    // huge tenant search fee can make denominator negative
    const result = getBreakEvenRent('150', '1200', '870', '5', '8', '0', 0, 12, 1);
    expect(result).toBe('0');
  });
});

describe('computeExitScenario with tenant search fee', () => {
  it('should match base behavior when tenant search fee is 0 (default)', () => {
    const base = computeExitScenario(10, 150000, 0, 2, 120000, 3.5, 25, 601, 42000, 900, 120, 1000, 5, 8, 1.5, 2, 3);
    const explicit = computeExitScenario(10, 150000, 0, 2, 120000, 3.5, 25, 601, 42000, 900, 120, 1000, 5, 8, 1.5, 2, 3, 0, 3);
    expect(base).not.toBeNull();
    expect(explicit).not.toBeNull();
    if (!base || !explicit) return;
    expect(explicit.cumulativeCashflow).toBe(base.cumulativeCashflow);
    expect(explicit.totalProfit).toBe(base.totalProfit);
  });

  it('should reduce cumulative cashflow when tenant search fee is set', () => {
    const noFee = computeExitScenario(10, 150000, 0, 2, 120000, 3.5, 25, 601, 42000, 900, 120, 1000, 5, 8, 1.5, 2, 3, 0, 3);
    const withFee = computeExitScenario(10, 150000, 0, 2, 120000, 3.5, 25, 601, 42000, 900, 120, 1000, 5, 8, 1.5, 2, 3, 1, 3);
    expect(noFee).not.toBeNull();
    expect(withFee).not.toBeNull();
    if (!noFee || !withFee) return;
    expect(withFee.cumulativeCashflow).toBeLessThan(noFee.cumulativeCashflow);
    expect(withFee.totalProfit).toBeLessThan(noFee.totalProfit);
  });

  it('should amortize: shorter tenancy = higher annualized cost = lower cashflow', () => {
    const longTenancy = computeExitScenario(10, 150000, 0, 2, 120000, 3.5, 25, 601, 42000, 900, 120, 1000, 5, 8, 1.5, 2, 3, 1, 6);
    const shortTenancy = computeExitScenario(10, 150000, 0, 2, 120000, 3.5, 25, 601, 42000, 900, 120, 1000, 5, 8, 1.5, 2, 3, 1, 1);
    expect(longTenancy).not.toBeNull();
    expect(shortTenancy).not.toBeNull();
    if (!longTenancy || !shortTenancy) return;
    expect(shortTenancy.cumulativeCashflow).toBeLessThan(longTenancy.cumulativeCashflow);
  });

  it('should treat tenant search fee months <= 0 as inactive', () => {
    const explicit = computeExitScenario(10, 150000, 0, 2, 120000, 3.5, 25, 601, 42000, 900, 120, 1000, 5, 8, 1.5, 2, 3, 0, 3);
    const negative = computeExitScenario(10, 150000, 0, 2, 120000, 3.5, 25, 601, 42000, 900, 120, 1000, 5, 8, 1.5, 2, 3, -1, 3);
    expect(explicit).not.toBeNull();
...
describe('computeStressScenarios with tenant search fee', () => {
  it('should match base behavior when tenant search fee is 0 (default)', () => {
    const base = computeStressScenarios(900, 120, 1000, 5, 601, 1.5, 25, 2, 0, 3, 42000, 150000, 1.5);
    const explicit = computeStressScenarios(900, 120, 1000, 5, 601, 1.5, 25, 2, 0, 3, 42000, 150000, 1.5, 0, 3);
    expect(explicit).toEqual(base);
  });

  it('should lower cashflow in all 3 scenarios when fee is set', () => {
    const noFee = computeStressScenarios(900, 120, 1000, 5, 601, 1.5, 25, 2, 0, 3, 42000, 150000, 1.5, 0, 3);
    const withFee = computeStressScenarios(900, 120, 1000, 5, 601, 1.5, 25, 2, 0, 3, 42000, 150000, 1.5, 1, 3);
    for (let i = 0; i < 3; i++) {
      expect(withFee[i].cashflowY1).toBeLessThan(noFee[i].cashflowY1);
      expect(withFee[i].totalReturn).toBeLessThan(noFee[i].totalReturn);
    }
  });

  it('should reduce DSCR when fee is set (lower net income / same mortgage)', () => {
    const noFee = computeStressScenarios(900, 120, 1000, 5, 601, 1.5, 25, 2, 0, 3, 42000, 150000, 1.5, 0, 3);
    const withFee = computeStressScenarios(900, 120, 1000, 5, 601, 1.5, 25, 2, 0, 3, 42000, 150000, 1.5, 1, 3);
    expect(Number(withFee[1].dscr)).toBeLessThan(Number(noFee[1].dscr));
  });
});
